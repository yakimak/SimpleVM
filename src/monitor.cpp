...
// Функция для разбиения строки на токены (CString String*)
...
void freeArgs(std::vector<String*>& args) {
...
// help
void printHelp(bool poweredOn) {
    std::cout << "\n=== SimpleVM Shell Commands ===" << std::endl;
    if (!poweredOn) {
        std::cout << "  poweron           - Power on computer" << std::endl;
...
void printStatus(Computer& computer) {
    ...
    std::cout << "CPU Mode: " << cpu.getModeBits() << "-bit" << std::endl;
    ...
    std::cout << "OS Loaded: " << (computer.isOSLoaded() ? "YES" : "NO") << std::endl;
    std::cout << "====================\n" << std::endl;
}
...
int Monitor::Run(Computer& computer) {
    std::string input;
    while (true) {
        std::cout << "SimpleVM> ";
        ...
        if (!computer.isPoweredOn()) {
            if (cstring_bridge::equalsLit(command, "exit") || cstring_bridge::equalsLit(command, "quit")) {
                ...
                freeArgs(args);
                break;
            } else if (cstring_bridge::equalsLit(command, "poweron")) {
                std::cout << "Powering on computer..." << std::endl;
                try {
                    computer.powerOn();
                    std::cout << "Computer is now powered on." << std::endl;
                } catch (const std::exception& e) {
                    std::cerr << "Power on failed: " << e.what() << std::endl;
                }
                freeArgs(args);
                continue;
                ...
                std::cerr << "Computer is powered off. Available commands: poweron, exit" << std::endl;
                freeArgs(args);
                continue;
            }
        }
        ...
        if (cstring_bridge::equalsLit(command, "exit") || cstring_bridge::equalsLit(command, "quit")) {
            ...
            if (args.size() < 2) {
                ...
                try {
                    for (int i = 0; i < steps; ++i) cpu.executeNext();
            ...
            std::cout << "Computer is already powered on." << std::endl;
        } else {
